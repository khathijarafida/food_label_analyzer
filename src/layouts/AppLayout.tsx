...
import {
  Leaf, LayoutDashboard, ScanLine, History, GitCompareArrows, Dumbbell,
  Search, User, Shield, LogOut, Moon, Sun, Menu, X,
} from 'lucide-react';
...
export default function AppLayout() {
  ...
  return (
    <div className="min-h-screen bg-botanical">
      ...
      <div className="bg-blob bg-mint-200/20 w-80 h-80 bottom-20 left-20 animate-float-blob" style={{ animationDelay: '2s' }} />
      {/* Desktop sidebar */}
      <aside className="hidden lg:flex fixed inset-y-0 left-0 w-64 flex-col p-4 bg-white/80 backdrop-blur-xl border-r border-primary-100 z-30">
        <div className="flex items-center gap-3 px-2 py-4">
          ...
          <div>
            <h1 className="font-display font-bold gradient-text leading-tight">NutriScan</h1>
            <p className="text-xs text-slate-500">AI Food Analyzer</p>
          </div>
        </div>
      ...
      <header className="lg:hidden sticky top-0 z-30 bg-white/80 backdrop-blur-xl border-b border-primary-100 px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h1 className="font-display font-bold gradient-text">NutriScan</h1>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={toggleTheme} className="p-2 rounded-lg hover:bg-primary-50 text-slate-600">
            {theme === 'dark' ? <Sun className="h-5 w-5" /> : <Moon className="h-5 w-5" />}
          </button>
          <button onClick={() => setMobileOpen(true)} className="p-2 rounded-lg hover:bg-primary-50 text-slate-600">
          ...
          <>
            ...
            >
              <div className="flex items-center justify-between px-2 py-4">
                ...
                <button onClick={() => setMobileOpen(false)} className="p-2 rounded-lg hover:bg-primary-50">
                  <X className="h-5 w-5 text-slate-600" />
                </button>
              </div>
              <nav className="mt-4 flex flex-col gap-1 flex-1" onClick={() => setMobileOpen(false)}>
                {navItems.map((item) => (
                  <NavLink key={item.to} to={item.to} className={() => navLinkClass(item.to)}>
                    <item.icon className="h-5 w-5" />
                    <span>{item.label}</span>
              ...
              <div className="border-t border-slate-200 pt-3 flex flex-col gap-1" onClick={() => setMobileOpen(false)}>
                <NavLink to="/profile" className={() => navLinkClass('/profile')}>
                  <User className="h-5 w-5" />
                  <span>Profile</span>
                </NavLink>
                <button onClick={handleSignOut} className="flex items-center gap-3 px-4 py-3 rounded-xl font-medium text-danger-600 hover:bg-danger-50 transition-all">
                  <LogOut className="h-5 w-5" />
                  <span>Sign Out</span>
                </button>
              </div>
